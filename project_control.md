...
| --- | --- | --- | --- |
| Interactive atlas | Expanded | Causal graph, node details, search, lineage view, fog, and timeline are implemented; the selected AI-for-Physics lineage and frontier comparators are now visible. | Run a local meeting-demo walkthrough and add deeper paper nodes after the import layer is reviewed. |
| Seed research content | Complete first integration | The visible graph now connects genetic programming, symbolic regression, neural operators, AI for Physics, world models, bounded RSI, and the selected project. | Expand only after evidence notes and relationship imports are reviewed. |
...
| Quality checks | Partial | Production build and lint pass as of 2026-09-18. There is no automated test suite. | Add interaction and accessibility tests. |
| Cloud development and delivery | Configured locally | Codespaces, GitHub Actions verification, and GitHub Pages deployment are configured. | Commit and push the configuration, then select GitHub Actions in the repository's Pages settings. |
